...
async function generateMonthlySummaryImage(siteName: string, summaryStats: Record<string, number>, reportTitle: string, startDate: string, endDate: string): Promise<string> {
  const container = document.createElement('div');
...
export async function processMonthlyFile(file: File): Promise<ProcessResponse> {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (minDate && maxDate) {
           const m1 = minDate.toLocaleString('default', { month: 'short' }).toUpperCase();
           const y1 = minDate.getFullYear();
           const m2 = maxDate.toLocaleString('default', { month: 'short' }).toUpperCase();
        ...
        for (const site of siteKeys) {
          ...
          const listDataUrl = await generateMonthlyListImage(site, rows, dateLabel, startDateStr, endDateStr);
          const listFilename = `${site.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_site_visit.png`;
          ...
          const summaryDataUrl = await generateMonthlySummaryImage(site, summaryStats, dateLabel, startDateStr, endDateStr);
